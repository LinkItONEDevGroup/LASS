         ...
         if (len > 0) {
            ...
            if (type == MQTTCONNECT) {
               if (callback) {
               	  ...
               	  uint8_t tl_offset=0;
               	  if((buffer[1] & 0x80)>0) {tl_offset=1;} //can't handle remain length > 16383
                  uint16_t tl = (buffer[tl_offset+2]<<8)+buffer[tl_offset+3];
                  
                  char topic[tl+1]; //not sure this memory will dynamic allocated
                  for (uint16_t i=0;i<tl;i++) {
                     topic[i] = buffer[tl_offset+4+i];
                  }
                  topic[tl] = 0;
                  // ignore msgID - only support QoS 0 subs
                  uint8_t *payload = buffer+tl_offset+4+tl;
                  callback(topic,payload,len-tl_offset-4-tl);
               }
...
boolean PubSubClient::publish(char* topic, uint8_t* payload, uint16_t plength) {
   return publish(topic, payload, plength, false);
}

boolean PubSubClient::publish(char* topic, uint8_t* payload, uint16_t plength, boolean retained) {
   if (connected()) {
      // Leave room in the buffer for header and variable length field
      uint16_t length = 5;
...
/*
boolean PubSubClient::publish_P(char* topic, uint8_t* PROGMEM payload, uint16_t plength, boolean retained) {
   uint8_t llen = 0;
   uint8_t digit;
   int rc;
   uint16_t tlen;
