...
void loop() {
  ...
  /*
     一個循環約4-5秒，因此取8秒緩衝空間判定需上傳/寫入資料
  */
  bool _SD_save = now.unixtime() % SD_save_interval <= 5;    //  5分鐘存檔一次
  bool _upload = now.unixtime() % Upload_interval <= 5;       // 20分鐘上傳一次
